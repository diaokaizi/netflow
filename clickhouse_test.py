...
def run_threaded(job_func, host):
    job_thread = threading.Thread(target=job_func,args=(host,))
    job_thread.start()

if __name__ == '__main__':
    host = "223.193.36.79:7140"
    creat_table()
    schedule.every(5).minutes.do(run_threaded, insert_data_clickhouse, host)
    while True:
